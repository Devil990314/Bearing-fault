...
n_imfs = 4;
imf11=imf';
%plot(time_vect,imf(:,1:n_imfs) - 2*repmat(1:n_imfs,size(imf1,1),1))
plot(time_vect,(imf11(:,1:n_imfs) - 2*repmat(1:n_imfs,size(imf11,1),1))');
yticks(-2 * (7:-1:1))
yticklabels({'IMF7','IMF6','IMF5','IMF4','IMF3','IMF2','IMF1'})
title('Intrinsic Mode Functions')
xlim([3 5])
ax = subplot(7,1,[5 6 7]);
plot_hht( imf11, S.sample_rate, ax, 350);
...
%plot(dataFIC.time{1},(imf11(1:n_imfs) - 1e-12*repmat(1:n_imfs,size(imf11,1),1))')
plot(dataFIC.time{1},(imf11(1:n_imfs) - 1e-12*repmat(1:n_imfs,size(imf11,1),1))')
yticks(-1e-12 * (5:-1:1))
