function feature_matrix=Copy_of_Export_feature(data)
%2022.3.14版本 可计算各个熵值大小
% 计算一组样本的特征，并将这些特征保存到一个 Excel 文件中 2022.3.14版本
% 初始化特征矩阵
...
assignin('caller','Inverter_fre',fr);%将转频返回至主函数

disp('样本标签序号为： 0.Norm    1.OR     2.IR');
label_num=input('请输入标签序号: ');
%调用switch_lable_num函数
Sample_label=switch_lable_num(label_num);

% 计算时域特征
for i = 1:size(data,2)
    tic;disp(['正在进行第',num2str(i),'个样本的特征提取']);
    %共17个时域特征
    ...
    eDim=2;r0=0.15;FuzEn = FuzzyEntropy(data(:,i)',eDim,r0,2,1);%模糊熵
