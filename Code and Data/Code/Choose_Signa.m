...
if i==5 && 2
    ...
    switch i1
        case 1
            ...
            data_2 = Fun_process_signal_designfilt(data_3Sigma, fs, 1000);%对信号进行滤除基波处理
            assignin('base','data_3Sigma',data_3Sigma);%将输出返回至主函数
            data_positive = data_2 - min(data_2);%归0处理，保证所有数据都是正值
            assignin('base','data_positive',data_positive);%将输出返回至主函数
        case 2
            data_3Sigma = Fun_3Sigma(data,fs);%对信号进行3西格玛处理
            assignin('base','data_3Sigma',data_3Sigma);%将输出返回至主函数
            data=data_3Sigma;
            data_positive = data - min(data);%归0处理，保证所有数据都是正值
            assignin('base','data_positive',data_positive);%将输出返回至主函数
        otherwise
            disp("------------滤波选择输入错误！")
    end
