function filtered_signal = Fun_process_signal_designfilt_new(input_signal, fs, bw, low_pass_cutoff)
    ...
    % 预热阶段
    warmup_duration = 1; % 预热持续时间（秒）
    warmup_samples = round(warmup_duration * fs); % 预热持续的样本数
    warmup_signal = filtered_signal(1:warmup_samples); % 提取预热信号段
    for harmonic = 1:max_harmonic
        ...
        % 应用滤波器（预热阶段）
        warmup_signal = filtfilt(d, warmup_signal);
    end
