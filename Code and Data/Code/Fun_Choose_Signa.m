function [data_all,data, fs,fr,Fre_Bearing]=Fun_Choose_Signa(i)
%0901修改
switch i
    case 1
        disp("------------开始分析 振动信号(b216) ------------ ");
        ...
        data_all=Fun_load_B216_data(ii);
        disp("------------振动数据(b216)加载成功------------ ");
        fr=input('------------请输入转速/rpm: ');
        fs=10*1000;Fre_Bearing=Fun_Bearing_Fre(6205,fr);
    case 2
        disp("------------开始分析 电流信号(b216)------------ ");
        disp('------------请选择要分析的电流信号(b216)------------')
        data_all=Fun_load_B216_data(2);
        disp("------------电流信号(b216)加载成功------------ ");
        fs = input('------------请输入采样频率/kHz: ');
        fr = input('------------请输入转速/rpm: ');
        fs=fs*1000;Fre_Bearing=Fun_Bearing_Fre(6205,fr);
    case 3
        disp("------------开始分析 CWRU信号------------ ");
        disp('------------请选择要分析的CWRU信号------------');
        [data_all,fs,fr]=Fun_load_Xichu_data;
        disp("------------CWRU信号加载成功------------ ");
        Fre_Bearing=Fun_Bearing_Fre(6205,fr);
    case 4
        disp("------------开始分析 Paderborn信号------------ ");
        [data_all,fs,fr] =Fun_load_Paderborn_data();
        disp("------------Paderborn信号加载成功------------ ");
        Fre_Bearing=Fun_Bearing_Fre(6203,fr);
    case 5
        disp("------------开始分析 202306 Current信号------------ ");
        [data_all,fs,fr] =Fun_load_202306_Current_data();
...
if i==5 && 2
    ...
    switch i1
        case 1
            ...
            data_Filter_Fun_wave = Fun_process_signal_designfilt(data_3Sigma, fs, 0.5,1000);%对信号进行滤除基波处理
