...
% init = 1;          
% tol = 1e-5;
% [u, u_hat, omega] = VMD(data_select, alpha, tau, K, DC, init, tol);
% 
% figure;%建立幕布
% for k=1:K%IMF
%     subplot(K+3,1,k);plot(u(k,:),'k'); 
% end
% signal2=zeros(1,size(data_select,1) );%去噪后的信号
% for k=1:K
%     if  max(u(k,:))>10
%         signal2=signal2+u(k,:);
%     end
% end
% subplot(K+3,1,k+1);plot(data_select,'k');%原始信号 
